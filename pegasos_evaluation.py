...
class Pegasos():
    ...
    def train(self):
        ...
        for t in range(1,self.iter+1):
            ...
            for j in range(len(self.G_train)):
                sigma_loss += alpha[j] * self.y_train[j] * gkf.GraghkernelFunc.k_func_wl(self.G_train[i_t], self.G_train[j], 1)
    ...
    def predict(self, alpha):
        ...
        for i in range(len(self.G_test)):
            y = 0
            for j in range(len(self.G_train)):
                y +=  alpha[j] * self.y_train[j] * gkf.GraghkernelFunc.k_func_wl(self.G_test[i], self.G_train[j], 1)
